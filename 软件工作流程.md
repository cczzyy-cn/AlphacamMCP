...
> **关于 CDM 批量处理**：CDM 没有暴露任何 `Public` 级别的"批量生产"VBA 函数。Processing 流程完全绑定在 `frmNTCW` 窗体的 UI 事件中，`Make` 模块的所有加工函数（`mbln_Style_xxx`、`mbln_MakeMachining` 等）均为 `Private`，无法通过 `run_vba_macro` 直接调用。但订单创建（数据库写入）可完全自动化。
...
> ⚠️ **CDM 架构约束**：经过源码分析，CDM 没有任何 `Public` 级别的"一键批量处理"VBA 函数可供 `run_vba_macro` 调用。Processing（几何生成+刀路应用）完全通过 `frmNTCW` 窗体 UI 驱动。因此采用**分段自动化**策略。

#### 方案 A：订单自动化 + CDM UI Processing（推荐）

| 环节 | 自动化 | 方式 |
|------|--------|------|
| ① 创建订单 + 导入门板数据 | ✅ **完全自动** | `BatchImport.Run` → 直接写 CDM 数据库 |
| ② Processing（生成几何+刀路） | ⚠️ **需 CDM UI** | `run_vba_macro("CDM.m_Processing")` 打开窗体，用户点击 Process |
| ③ 刀具路径排序 | ✅ **可调用** | `run_vba_macro("CDM.m_OrderToolPaths")` |
| ④ NC 输出 | ✅ **MCP 工具** | `select_post` + `output_nc` |
...
```python
# ══════════════════════════════════════════════════════════
# CDM 橱柜门完整工作流：CSV 导入 → CDM Processing → NC 输出
# ══════════════════════════════════════════════════════════

# ── 阶段 1：订单自动化（完全自动）──
# 安装 BatchImport 模块（只需首次）
code = open("CDM功能/BatchImport.bas", "r", encoding="utf-8").read()
install_vba_module(module_name="BatchImport", code=code)

# 导入 CSV → 创建订单 + 门板数据写入 CDM 数据库
run_vba_macro(
    macro_name="CDM.BatchImport.Run",
    params=["C:\\Users\\C\\Desktop\\2026优化表\\7-10中林SPC婷兰灰.csv",
            "7-10中林SPC婷兰灰"],
...
# ── 阶段 2：Processing（需人工确认）──
# 打开 CDM Processing 主界面
# ⚠️ 用户需要在窗体中：
#    1. 选择刚创建的订单
#    2. 点击 "Process" 按钮
#    3. CDM 自动生成几何图形 + 应用刀路 + 排版嵌套
run_vba_macro(macro_name="CDM.m_Processing")


# ── 阶段 3：排序 + NC 输出（可自动）──
# 刀具路径排序（按 Sheet 排序 + 末刀后置）
