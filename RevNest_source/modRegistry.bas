in file: C:\Program Files (x86)\Vero Software\Alphacam 2016 R1\000\StartUp\Utils\ReverseNest\ReverseNest.amb - OLE stream: 'vao/The VBA Project/_VBA_Project/VBA/modRegistry'
- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
Option Explicit
...
' >< CONSTANTS ><
'
Public Const DEF_REG                        As String = "Software\Planit\Alphacam"
Public Const DEF_REG_KEY                    As String = DEF_REG & "\" & DEF_MACRO_NAME
'
...
Private Declare PtrSafe Function RegSaveKey Lib "advapi32.dll" Alias "RegSaveKeyA" (ByVal hKey As LongPtr, ByVal lpFile As String, lpSecurityAttributes As Any) As Long
...
On Error Resume Next
        ...
        lngRet = RegSaveKey(lngHKey, sFileName, ByVal 0&)
...
On Error Resume Next
        ...
        lngRet = RegCreateKeyEx(lKeyRoot, sKeyPath, 0, "", 0, KEY_WRITE, SA, lngHKey, lngNewKey)
        ...
        Select Case lKeyType
                Case REG_SZ
                        lngRet = RegSetValueEx(lngHKey, sSubKey, 0, lKeyType, ByVal sSubKeyValue, Len(sSubKeyValue))
                Case REG_DWORD
                        lngRet = RegSetValueEx(lngHKey, sSubKey, 0, lKeyType, CLng(sSubKeyValue), 4)
                Case REG_BINARY
                        lngRet = RegSetValueEx(lngHKey, sSubKey, 0, lKeyType, CByte(sSubKeyValue), 4)
        End Select
...
Public Function gs_EnumerateRegKeyValues(ByVal lKeyRoot As KeyRoot, ByVal sKeyPath As String) As String
