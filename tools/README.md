...
| 步骤 | 工具 | 作用 |
|---|---|---|
| 1 | `running_snapshot.py [tag]` | 部署前把**运行中**的组件导出到 `backup/`，作为回滚点与审计基线 |
| 2 | `component_deploy.py audit <组件> <仓库相对路径> <基线 glob>` | 比对「运行中 / 仓库 / 基线」。**只有运行版等于仓库或等于基线时才允许写**，防止覆盖掉别人在 VBA 编辑器里的手改 |
| 3 | `component_deploy.py deploy ...` | `DeleteLines` + `AddFromString` 整体替换，**读回校验**，校验失败自动回滚 |
| 4 | `deploy_probe.py` | 断言新版标记在位、旧代码已消失，并用 `App.Run` 触发**全工程编译**探针 |
