...
### 7.14 `ManualToolPath` 重建出来的刀路**不在任何版件里** → 加工道次窗口出现「(不在版件中)」（2026-09-14 真机实测，重要）

**现象**（用户在真实排版工件上跑完 v2.2.0 后反馈）：加工道次窗口（按【版件】分组）显示
...
**根因**：`MillData.ManualToolPath` + `Finish` 造的是**全新**刀路，它们**没有进入排版版件的刀路表**。
刀路仍然能加工（NC/道次都正常），但"属于哪个版件"这个归属丢了。

**证据（签名交叉比对，`tmp/nest_match.py`；不能靠 `Path.Sheet`，见下）**：
...
即 `NestSheet.Paths`（版件1=44 条刀路、版件2=16 条）恰好只装那 60 条原始刀路。

**踩过的查询陷阱**：`Path.Sheet` 在 typelib 里是 `(1006, …, (11,0))` = **VT_BOOL**（不是版件对象！），
而且对**所有**刀路都返回 `False`。第一版脚本把它当对象取 `.Number`，于是"80 条全都不在版件中"，
差点得出完全相反的结论 —— **判据错会给出看起来很确定的错答案**，务必用交叉比对复核。

**版件归属没有 API 可以补**：typelib 里根本没有 `INestSheet` 接口（`NestSheet` 是无类型信息的对象），
`Drawing` 上也没有 "AddPathToSheet" 之类。所以**不能事后把刀路塞回版件**。

#### 解法（已验证的机制）：`Path.ReplaceElements(Path2)` —— 原地换元素，保留刀路身份

文档：*Replace the elements in the current path with those in given path. **Path2 will be deleted.***

因为**刀路对象本身没被换掉**（只是换了它的元素），版件表里的引用依然有效 →
**版件归属、子工序、道次 `OpNo`、加工数据(MillData) 全都保住**。真机实测（`tmp/exp_replace_elements.py`）：

| 检查项 | 结果 |
|---|---|
| 版件归属 | 前 `版件1` → 后 `版件1` ✅ |
| 刀路总数 | 前 80 → 后 80 ✅（副本被消耗） |
| 元素内容 | 逐元素一致 ✅ |
| 加工数据 `MillData`（进给/转速/深度/安全高度/刀具号…） | **完全没变** ✅ |
| 子工序 / `OpNo` / 结构键 | 不变 ✅ |

**两个必须自己存回的字段**（`ReplaceElements` 会把**源刀路**的标志带过来）：

- `ToolInOut`（**可写**）：实测被源刀路的值覆盖（`0 → 1`）。这个字段影响进出刀行为，**必须存回**；
- `Closed`（**只读**，`_prop_map_put_` 里没有）：实测 `False → True`，**改不回来** ——
  要么接受，要么用"源刀路自己构造"的方式让它取到期望值。

其余可写且值得存回的：`NoRapidUpBefore`、`CompOnRapid`、`Group`、`Color`、`ToolSide`。

> **结论**：`modRamp` 若要从"删掉旧刀路、另建新刀路"改成"**原地 ReplaceElements**"，
> 就能一次消掉：`(不在版件中)`、子工序数量爆炸（工序5 从 1 个变 7 个）、
> 以及"新建子工序只能叫手动输入"这一整类问题。
> 但注意 `Finish`/`ReplaceElements` 都会**在 rapid/feed 边界切分刀路**，
> 结尾的抬刀 rapid 仍可能被单列成一条独立刀路（§7.9(g)）。

#### 已实施：`modRamp` v2.3.0 —— 原地重建（2026-09-14，真机验收通过）

改法（`CCC功能/modRamp.bas`）：

```vba
Set newPaths = mtp.Finish
If newPaths.Count >= 2 Then                  ' 抬刀被单独切出来了 -> 并回主干
    For q = newPaths.Count To 2 Step -1
        newPaths(1).AddPath newPaths(q)       ' 消耗 newPaths(q)
    Next q
End If
tp.ReplaceElements newPaths(1)               ' 元素换进【原刀路】, 身份不变
tp.ToolInOut = svToolInOut                   ' ReplaceElements 会带入源标志 -> 写回
```

**真机验收**（真实排版工件：2 版件 / 10 工序；`小条范围=400 / 深 18.06 / 角 15°` → 受理 10 条）：

| 判据 | v2.2.0（重建新刀路） | **v2.3.0（原地重建）** |
|---|---|---|
| 刀路总数 | 70 → **80**（每件多一条抬刀） | 70 → **70** ✅ |
| 子工序数（工序5 / 工序10） | 1 → **7** / 1 → **4** | **1 → 1 / 1 → 1** ✅ |
| 版件归属 | 20 条里 **0** 条在版件里 | **10/10 全部在版件里** ✅ |
| 加工道次窗口 | 出现 `(不在版件中)` | `(不在版件中)` **消失**（截图核对）✅ |
| 抬刀 | 单列成一条纯 rapid 刀路 | 在同一条刀路的**末尾** ✅ |
| 零长重复元素 | 9/10 条中招 | **0** ✅ |
| 斜坡锚定 / 切深 | 102.08mm / -18.06 | 67.4mm（角 15°）/ -18.06 ✅ |
| 幂等（同参数复跑） | — | `已处理过=10`，结构逐条不变 ✅ |

**顺带又踩到两个坑（都已记进代码/本文档）**：

1. **`VBE.ActiveVBProject` 可能指向别的插件**（本次是**受保护的 CDM**）→
   `VBComponents.Add` 直接报"该工程已被保护"，把人往"工程被保护"的错误方向带。
   **正确做法：按工程名显式查找**（`VBProjects(i).Name = "CCC功能"`）。
2. **深度门槛的浮点坑**：版件厚度 18.06 存成单精度后，刀路 `FinalDepth = 18.059999465942383`。
   用户在窗体里按**名义厚度 18.06** 填门槛时，`Abs(origDepth) >= cutDepth` 会**一分不差地判否**，
   结果是"候选 70 / 深度不足 42 / 一条都不处理"，提示里完全看不出是浮点问题。
   → v2.3.1 给门槛加了容差 `DEPTH_EPS = 0.05mm`。**凡是"用户填的名义值 vs 软件存的浮点值"比较，都要留容差。**

#### 顺带记录：`App.CreateTempDrawing` **不能**当刀路沙箱（我踩了）

`App.CreateTempDrawing()` 的文档说"创建一个不显示的临时图纸，可用作工作区"，
但 **`App.CreateMillData` / `MD.ManualToolPath` / `App.SelectTool` 都作用在【活动图纸】上** ——
在临时图纸存在时调用它们，刀路会落到**用户正在编辑的真实图纸**里（我因此往用户工件里
凭空加出一个"工序 11 + 测试方框刀路"，已删除还原）。

**判定"哪些是我误加的"**：比较工序 `Number` 与实验前的工序数（凭空多出来的号段）+ 刀具名 +
坐标范围，**不要只看包围盒**（首元素常被自动加的引导 rapid 撑大，看起来像野坐标）。
