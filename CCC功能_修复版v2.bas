' ==============================================================================
' CCC功能 合集 — UserForm 版（修复二次排序失效 v2）
' ==============================================================================
' 修复策略：
'   保留原算法"按操作索引分组"的结构（不走按条数均分的错误路线），
'   只针对性修复二次排序失效的 3 个 root cause。
' ==============================================================================
...
' 修复 ApplySortToDrawing：保留原算法结构，只修二次排序失效
Public Sub ApplySortToDrawing(ByRef sortedKeys() As String)
    ...
    ' 如果没检测到排版几何，按一张排版处理（所有操作统一排序）
    Dim totalOps As Long: totalOps = ops.count
    Dim opsPerSheet As Long: opsPerSheet = totalOps
    If sheetCount > 0 And (totalOps Mod sheetCount) = 0 Then
        opsPerSheet = totalOps / sheetCount
    ElseIf sheetCount > 0 Then
        ' 不能整除时，按 sheet 数量尽量均匀分配
        opsPerSheet = Int(totalOps / sheetCount) + 1
    End If
