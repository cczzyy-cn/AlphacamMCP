...
- 点击"重新生成标签" → ???? **操作的是当前打开的图纸**，需先在 AlphaCAM 中打开排版档案 ARD 文件（如 `<订单>_<材料>.ard`，含嵌套信息）；否则会提示"当前图纸不是排版档案"并拒绝。手动移动/删除门板后使用：
  0. **先弹「未保存修改」确认**（v1.7）：未保存的手动移动只用于出标签、不会写回原图，重生成后会丢失 —— 建议先 Ctrl+S
  1. **备份当前图纸状态与原 ARD 文件**（临时副本，保护加工道次窗口不被破坏）
  2. 把该订单/材料的旧件标签 EMF 移到备份目录（清理残留；失败时可整体还原）
  3. 按当前图纸位置重新生成 `<JobName>_<材料>_<板名>_<件号>.emf`
  4. **同步数据库**（事务 `BeginTrans`/`CommitTrans`）：按 `DEF_ATT_DETAIL_ID` 更新 `AD_REPORT_DATA.PressDoorImage/PressDoorCounter`，并删除已删板件的记录
  5. **恢复用户图纸**：重开用户原 ARD —— 原文件内容不变，加工道次窗口状态不受影响
  6. **失败时（v1.7）**：回滚数据库 + 从备份目录还原旧 EMF + 删除 `regen_*` 临时文件 + 重新打开用户原档案
