...
Public Sub ApplySortToDrawing(ByRef sortedKeys() As String)
    ...
    Dim mSheet As Long, pos As Long, lastSh As Long
    Dim spInt As Integer
    
    ' ===== 棣栨鎺掑簭锛歂estInformation 鍖归厤锛屾寜璺緞鍚嶄繚瀛?=====
    If g_mapPathToSheet Is Nothing Then
        ...
        ' 閬嶅巻 OP锛屽尮閰嶆帓鐗堬紝鎸夌涓€涓矾寰勫悕淇濆瓨鏄犲皠
        For opIdx = 1 To ops.count
            ...
            ' 鍙栫涓€涓矾寰勫悕
            Dim firstTpName As String: firstTpName = ""
            ...
            ' 鎸夎矾寰勫悕淇濆瓨鏄犲皠锛堢ǔ瀹氾紝OrderAll涓嶆敼璺緞鍚嶏級
            If firstTpName <> "" Then
        ...
        Dim keyVar
        For Each keyVar In g_mapPathToSheet.Keys
            Dim sv As Long: sv = g_mapPathToSheet(keyVar)
            If Not uniqS.Exists(CStr(sv)) Then uniqS.Add CStr(sv), True
        Next keyVar
        sheetCount = uniqS.count
        If sheetCount = 0 Then sheetCount = 1
    Else
        ' 鍚庣画鎺掑簭锛氫粠宸叉湁鏄犲皠鎺ㄧ畻鎺掔増鏁?
        Dim uniq2 As Object: Set uniq2 = CreateObject("Scripting.Dictionary")
        For Each keyVar In g_mapPathToSheet.Keys
            sv = g_mapPathToSheet(keyVar)
            If Not uniq2.Exists(CStr(sv)) Then uniq2.Add CStr(sv), True
        Next keyVar
        sheetCount = uniq2.count
        If sheetCount = 0 Then sheetCount = 1
    End If
    
    ' ===== 鎸?(鎺掔増, 鍒€鍏? 鍒嗙粍 =====
    ' 鐢ㄥ綋鍓?OP 鐨勭涓€涓矾寰勫悕鏌ユ槧灏勮幏鍙栨帓鐗堝彿
    ...
    For opIdx = 1 To ops.count
        ...
        If Not (sbM Is Nothing) Then
            ' 鏌ユ帓鐗?
            Dim lookupName As String: lookupName = ""
            If sbM.count > 0 Then
    ...
    ' ===== 鍒嗛厤 OpNo =====
    App.SetUndoCommandName "鎺掔増鍒€鍏锋帓搴?"
    App.SetUndoPoint
...
ErrHandler3:
    ...
    MsgBox "鎺掔増鍒€鍏锋帓搴忓嚭閿欙細" & Err.Description, vbCritical
End Sub